...
userSchema.pre('save',async function(next){
    ...
    // That password gets hashed and saved.
    
    // Later, the user updates their profile (e.g., changes name or email, but not password).

    if(!this.isModified('password')) return next();           
    // encryption of password.......
   this.password = await bcrypt.hash(this.password,5);
   // delete confirmpassword before saving it to the document.
